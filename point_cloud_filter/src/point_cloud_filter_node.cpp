...
public:
	...
	std::string input_topic;
        ...
        pn.param<double>("z_cutoff", z_cutoff_, 2.00);
        pn.param<double>("voxel", voxel_, 0.01);
        pn.param<bool>("noise", noise_, false);
        pn.param<std::string>("input", input_topic, "/camera/depth_registered/points");
        pn.param<std::string>("output", output_topic, "/camera/depth_registered/points/filtered");

        pc_sub_= nh_.subscribe<sensor_msgs::PointCloud2> (input_topic, 1, &PointCloudFilterNode::pcCallback, this);
        pc_pub_ = nh_.advertise<pcl::PointCloud<pcl::PointXYZRGB> > (output_topic, 2);
